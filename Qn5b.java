...
public class Qn5b extends JFrame {
    ...
    private static final String BASE_URL =
        "https://api.openweathermap.org/data/2.5/weather?q=%s&appid=%s&units=metric";  //api interface

    // Five Nepali cities to fetch weather for
    ...
    // Stores weather result for one city
    static class WeatherData {
        ...
        WeatherData(String city, String emoji) {
            this.city = city; this.emoji = emoji;
        }
    }
    ...
    private final Object tableLock = new Object(); // prevents two threads writing the table at once
    ...
    private JPanel buildHeader() {
        ...
        btnClear = makeBtn("X  Clear",           new Color(140, 140, 160));
    ...
    private JPanel buildWeatherTab() {
        ...
        // Pre-fill rows with dashes until real data arrives
    ...
    private void startFetching() {
        ...
        // Show loading spinner in every data cell
        ...
        // Background thread keeps GUI responsive during fetching
        new Thread(() -> {

            // Phase 1: fetch cities one by one to measure sequential time
            ...
            // Phase 2: fetch all cities at the same time using 5 threads
            ...
            for (String[] city : CITIES) {
                ...
                executor.submit(() -> {
                    log(String.format("  [PAR] %s → fetching %s",
                    ...
                    parQueue.add(wd);
                    results.put(name, wd);
                    log(String.format("  [PAR] %-12s → %dms [%s]",
                        name, wd.fetchTimeMs, wd.success ? "OK" : wd.error));

                    // Update table row safely from the UI thread
            ...
            log("Parallel total: " + parLatency + "ms  |  Speedup: "
                + String.format("%.2fx", speedup));

            SwingUtilities.invokeLater(() -> {
                progressBar.setValue(CITIES.length);
                progressBar.setString("[OK] Done!");
    ...
    private void fetchWeather(WeatherData wd) {
        try {
            String urlStr = String.format(BASE_URL,
                URLEncoder.encode(wd.city, "UTF-8"), API_KEY);
            ...
            simulateData(wd); // fall back to fake data on any error
        }
    }
    ...
    // Returns realistic fake data when the API is unavailable
    private void simulateData(WeatherData wd) {
        ...
        wd.condition  = new String[]{"Clear sky","Few clouds","Overcast",
                                      "Light rain","Partly cloudy"}[r.nextInt(5)];
            ...
            for (int r = 0; r < tableModel.getRowCount(); r++) {
                if (tableModel.getValueAt(r, 1).equals(wd.city)) {
                    if (wd.success) {
                        ...
                        tableModel.setValueAt("[ERR] " + wd.error, r, 2);
                    }
                    break;
                }
            }
    ...
    static class ChartPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Top half: total sequential vs parallel bars
            int topBot = H / 2 - 20;
            ...
            // Bottom half: per-city breakdown
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> new Qn5b().setVisible(true));
    }
}